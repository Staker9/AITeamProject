...
class Agent:
    ...
    #    print(grid[self.x], grid[self.y])
        print("Agent Die!, Resetting agent...")
        self.x = 1
        self.y = 1
        self.has_gold = False
        self.is_alive = True
    ...
    def shoot(self, grid):# 이거 아직 안됨.. Q러닝 만들면 그때 마무리 해야할듯
        if self.arrows > 0:
            for direction in ['UP', 'DOWN', 'LEFT', 'RIGHT']:
                ...
                if 0 <= target_x < self.grid_size and 0 <= target_y < self.grid_size:
                    if 'Wumpus' in grid[target_x][target_y]:
                        print("Shot the Wumpus!")
                        print("Scream! Wumpus die")
                        grid[target_x][target_y].remove('Wumpus')
                        self.arrows -= 1  # 화살 개수 감소
                        return

        else:
            print("No arrows left! Cannot shoot anymore.")